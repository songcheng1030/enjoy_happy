...
class FriendRepository  implements FriendRepositoryInterface
{
    public function friendSuggestions($limit) {
        ...
        if (count($possible_friends) == 0) {
            $query = 'SELECT a.*, b.profile_image FROM users as a LEFT JOIN userinfos b ON a.id = b.user_id where a.id in (SELECT id FROM users WHERE id NOT IN ((SELECT friend_id FROM friendlists WHERE user_id = '.Auth::user()->id.')) AND status = 1 AND id != '.Auth::user()->id.') LIMIT '.$limit;
            // $query = "SELECT id FROM users WHERE id NOT IN ((SELECT friend_id FROM friendlists WHERE user_id = '.Auth::user()->id')) AND status = 1 AND id != '.Auth::user()->id'";
        }
        else {
