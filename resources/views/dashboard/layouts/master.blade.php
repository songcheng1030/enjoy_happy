...
<body class="sidebar-main-active right-column-fixed">
    ...
    <script type="text/javascript">
		...
		$(document).ready(function() {
			...
			socket.on('newMessage', (data) =>{
				...
				if (receiverId !== data.senderId) {
					...
					$("#chat_" + chatUser.id + " " + ".chat-content-d").append(newMsgElement);
				}
			})
		})
        ...
        function sendMsg(event, id) {
            let msgText
            if (event.keyCode == 13) {
                msgText = $("#chat_" + id + " " + "#sendMsgTxt").val();
                if (!msgText) return;
                const now = new Date();
                ...
                $("#chat_" + id + " " + ".chat-content-d").append(newMsgElement);

                $("#chat_" + id + " " + "#sendMsgTxt").val("");
		...
		function selectChatUser(chatUserId) {
            ...
            let newChat = `<div class="chat-block" id="chat_`+ chatUserId +`">
            ...
            if ($('#chat-msg').find('#chat_' + chatUserId).length == 0) {
                $('#chat-msg').append(newChat);
            }
            $('#closeChatBtn' + chatUserId).on('click', function() {
                $('#chat-msg').find('#chat_' + chatUserId).remove();
			...
			const profileLink = chatUserProfile ? "<?php echo url('images/profile') ?>" + "/" + chatUserProfile :
			"<?php echo url('assets/dashboard/img/default-avatar-1.png'); ?>";
			$("#chat_" + chatUserId + " " + ".chat_user_avatar").attr('src', profileLink);
			$("#chat_" + chatUserId + " " + ".chat_user_name").html(chatUser.name);
			...
			$.post("{{ route('messages-fetch') }}", data, (msgs, status) => {
				$("#chat_" + chatUserId + " " + ".chat-content-d").empty();
				let msgElements = "";
				for (let msg of msgs) {
				...
				$("#chat_" + chatUserId + " " + ".chat-content-d").append(msgElements);
			});
		}
