							...
							<div class="user-post-data">
								<div class="d-flex flex-wrap">
									...
									<div class="media-support-info mt-2">
										@if (isset($result->NewsfeedUser))
										<h5 class="mb-0 d-inline-block"><a href="{{ route('user-profile',encrypt($result->user_id)) }}" class="">{{ ucwords($result->NewsfeedUser->name) }}</a>
										</h5>
									@endif
												...
												<div class="dropdown">
													<span class="dropdown-toggle">
														<a href="javascript:void(0);" class="likePost" newsfeed_id="{{ $result->id }}" route="{{ route('newsfeed-like')}}" user_id="{{ $result->user_id }}" likes_id="{{ Auth::user()->id }}">
															...
															@if(isset($newlike->NewsfeedUser) && $newlike->NewsfeedUser->id !== Auth::user()->id)
															@php $hasMe = null; @endphp
															@continue;
															@else
											...
											<div class="total-like-block ml-2 mr-3">
												<div class="dropdown">
													...
													<div class="dropdown-menu" @if($result->NewsfeedLike->count() == 0) style="background: #fff; border: 0 none;" @endif>
														...
														@php $like = 0; @endphp
														@foreach($result->NewsfeedLike as $newlike)
														@if($like < 7 && isset($newlike->NewsfeedUser)) <a class="dropdown-item" href="#">{{ $newlike->NewsfeedUser->name }}</a>
															@endif
									...
									<li class="mb-2 reply_comment_add_{{ $comment->id }}" id="comment-el-{{ $comment->id }}">
										<div class="d-flex flex-wrap justify-content-start">
											...
											<div class="comment-data-block ml-3">
												...
												<div style="font-size: 0.9em"><a href="javascript:void(0)" id="view_replies_button_{{$comment->id}}" onClick="viewReplies({{$comment->newsfeed_id}}, {{$comment->id}}, '{{ route('view-more-comments') }}')">View Replies</a></div>
												@endif
												<!-- Reply Comment Form  -->
                                               <form class="comment-text align-items-center mt-3 comment-reply-form comment_reply_add_{{$comment->id}}" route="{{ route('comment_reply_add')}}" user_id="{{ Auth::user()->id }}" newsfeed_id="{{ $result->id }}" comment_id="{{$comment->id}}" id="">
													<textarea class="form-control rounded" id="comment-reply-text-{{ $comment->id }}" name="comment" placeholder="" required></textarea>

													<button class="badge badge-primary mt-2" id="submit" type="button" onClick="replyComment({{ $result->id }}, {{$comment->id}})">Post</button>
